package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      protected var var_184:Timer;
      
      protected var var_1432:uint = 0;
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_277,this.clientWindowDestroyed);
            this.var_259 = param2;
            this._working = true;
            this.var_1182 = param4;
            this.var_1183 = param5;
            this.var_184 = new Timer(40,0);
            this.var_184.addEventListener(TimerEvent.TIMER,this.operate);
            this.var_184.start();
         }
         return _loc6_;
      }
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1182 *= 0.75;
         this.var_1183 *= 0.75;
         if(Math.abs(this.var_1182) <= 1 && Math.abs(this.var_1183) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_259 != null)
         {
            this.var_259(this.var_1182,this.var_1183);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         {
            this.var_184.stop();
            this.var_184.removeEventListener(TimerEvent.TIMER,this.operate);
            this.var_184 = null;
         }
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_277,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
