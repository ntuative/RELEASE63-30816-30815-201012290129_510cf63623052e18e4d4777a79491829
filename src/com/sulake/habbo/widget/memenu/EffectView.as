package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private var var_155:IWidgetAvatarEffect;
      ...
      private var var_786:IWindow;
      
      public function EffectView()
      {
         super();
         this.var_184 = new Timer(1000);
         this.var_184.addEventListener(TimerEvent.TIMER,this.onUpdate);
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this.var_184.stop();
            this.var_184.removeEventListener(TimerEvent.TIMER,this.onUpdate);
            this.var_184 = null;
         }
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_155.isActive)
            {
               _loc2_ = this.var_155.secondsLeft / Number(this.var_155.duration);
               this.var_787.width = _loc2_ * this.var_440;
            }
            else
            {
               this.var_787.width = 0;
               this.var_184.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_785.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_785.text;
            if(this.var_155.isActive)
            {
               _loc2_ = this.var_155.secondsLeft;
               ...
               this.var_785.text = _loc1_;
            }
            else
            {
               this.var_785.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_155.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_155.isActive)
         {
         ...
         else
         {
            ...
            this.var_786 = this._window.findChildByName("effect_hilite");
            this.var_786.visible = false;
         }
         this.var_785 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_787 = this._window.findChildByName("loader_bar");
         if(this.var_787 != null)
         {
            this.var_440 = this.var_787.width;
            this.var_184.start();
            this.onUpdate();
         }
         if(this.var_155.icon)
         {
            this.setElementBitmap("effect_icon",this.var_155.icon);
         }
      }
