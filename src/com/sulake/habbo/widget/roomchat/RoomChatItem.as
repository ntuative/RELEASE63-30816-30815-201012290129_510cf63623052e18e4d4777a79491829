package com.sulake.habbo.widget.roomchat
{
   ...
   public class RoomChatItem
   {
      ...
      private static const const_878:Number = 18;
      
      private static const const_559:int = 6;
      
      private static const const_874:int = 6;
      
      private static const const_879:int = 35;
      ...
      public function RoomChatItem(param1:RoomChatWidget, param2:IHabboWindowManager, param3:IAssetLibrary, param4:String, param5:IHabboLocalizationManager, param6:String)
      {
         ...
         if(!var_207)
         {
            ...
            var_207.mouseTreshold = 0;
            var_207.setParamFlag(WindowParam.WINDOW_PARAM_FORCE_CLIPPING,true);
         }
         if(!var_401)
         {
            var_401 = this._windowManager.buildFromXML(this._assetLibrary.getAssetByName("bubble_shout").content as XML,1) as IRegionWindow;
            var_401.tags.push("roomchat_bubble");
            var_401.x = 0;
            var_401.y = 0;
            var_401.width = 0;
            var_401.background = true;
            var_401.mouseTreshold = 0;
            var_207.setParamFlag(WindowParam.WINDOW_PARAM_FORCE_CLIPPING,true);
         }
         if(!var_402)
         {
            var_402 = this._windowManager.buildFromXML(this._assetLibrary.getAssetByName("bubble_whisper").content as XML,1) as IRegionWindow;
            var_402.tags.push("roomchat_bubble");
            var_402.x = 0;
            var_402.y = 0;
            var_402.width = 0;
            var_402.background = true;
            var_402.mouseTreshold = 0;
            var_207.setParamFlag(WindowParam.WINDOW_PARAM_FORCE_CLIPPING,true);
         }
         if(!var_342)
         {
      ...
      {
         this.var_1400 = param1.chatType;
         this.var_1147 = param1.userId;
         this.var_908 = param1.userName;
         this.var_1148 = param1.userCategory;
         this._message = param1.text;
         this.var_492 = param1.links;
         this.var_1810 = param1.userX;
         this.var_249 = param1.userImage;
         this.var_2551 = param1.userColor;
      ...
      public function set y(param1:Number) : void
      {
         ...
         if(this._window != null)
         {
            this._window.y = param1 - this.var_744;
         }
      }
      ...
      {
         var _loc5_:Rectangle = new Rectangle(this._x,this.var_161,this.width,const_878);
      ...
      public function renderView() : void
      {
         ...
         if(this.var_249 && !isRespect)
         {
            ...
            if(userIconWindow)
            {
               ...
               userIconWindow.x = x1;
               userIconWindow.y = y1;
               this.var_744 = Math.max(0,(this.var_249.height - background.height) / 2);
               this._width += userIconWindow.x + this.var_249.width;
            }
         }
         if(nameWindow != null)
         {
            if(!isRespect)
            {
               nameWindow.text = this.var_908 + ": ";
               nameWindow.y += this.var_744;
         ...
         if(this.var_1400 == RoomWidgetChatUpdateEvent.const_287)
         {
            textWindow.text = this._localizations.registerParameter("widgets.chatbubble.respect","username",this.var_908);
            this._width = const_879;
         }
         else if(this.var_1400 == RoomWidgetChatUpdateEvent.const_279)
         {
            textWindow.text = this._localizations.registerParameter("widget.chatbubble.petrespect","petname",this.var_908);
            this._width = const_879;
         }
         else if(this.var_492 == null)
         {
         ...
         else
         {
            ...
            lastLinkEndPos = -1;
            i = 0;
            while(i < this.var_492.length)
            {
               filteredLink = this.var_492[i][1];
               placeHolder = "{" + i + "}";
               placeholderPos = this._message.indexOf(placeHolder);
               lastLinkEndPos = placeholderPos + filteredLink.length;
               this.var_743.push([placeholderPos,lastLinkEndPos]);
         ...
         if(pointerWindow != null && pointerWindow.visible)
         {
            ...
            pointerWindow.y += this.var_744;
         }
         var bitmap:BitmapData = this.buildBubbleImage(nameWindow.width + textWindow.width,background.height,this.var_2551,isRespect);
         this._window.width = bitmap.width;
         this._window.y -= this.var_744;
         this._width = this._window.width;
         background.bitmap = bitmap;
         background.y = this.var_744;
      }
      ...
      private function onMessageLinkMouseClick(param1:WindowMouseEvent) : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this.var_743.length)
         {
            if(_loc2_ >= this.var_743[_loc3_][0] && _loc2_ <= this.var_743[_loc3_][1])
            {
               if(this.var_492[_loc3_][2] == 0)
               {
                  HabboWebTools.openExternalLinkWarning(this.var_492[_loc3_][0]);
               }
               else if(this.var_492[_loc3_][2] == 1)
               {
                  HabboWebTools.openWebPage(this.var_1811 + this.var_492[_loc3_][0],"habboMain");
               }
               else
               {
                  HabboWebTools.openWebPage(this.var_1811 + this.var_492[_loc3_][0]);
               }
            }
            _loc3_++;
         }
      ...
      {
         this._widget.onItemMouseClick(this.var_1147,this.var_908,this.var_1148,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseDown(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseDown(this.var_1147,this.var_1148,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseOver(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseOver(this.var_1147,this.var_1148,this._roomId,this._roomCategory,param1);
      }
      
      private function onBubbleMouseOut(param1:WindowMouseEvent) : void
      {
         this._widget.onItemMouseOut(this.var_1147,this.var_1148,this._roomId,this._roomCategory,param1);
      }
