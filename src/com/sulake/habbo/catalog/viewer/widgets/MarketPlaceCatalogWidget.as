package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class MarketPlaceCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener, IMarketPlaceVisualization
   {
      ...
      private const const_1843:int = 10;
      
      private var var_717:Array;
      
      private var var_1346:MarketPlaceOfferData;
      ...
      private function updateList() : void
      {
         ...
         this.updateStatusDisplay(this.const_1553,_loc2_.length);
         if(!this.var_475)
         {
      ...
      private function populateList() : Boolean
      {
         ...
         while(_loc1_ < 5)
         {
            if(this.var_1089 >= this._offers.length)
            {
               return true;
            }
            this.addListItem(this._offers.getWithIndex(this.var_1089));
            ++this.var_1089;
            _loc1_++;
         }
         return false;
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "back_button":
                  ...
                  break;
               case "buy_button":
                  this.marketPlace.buyOffer(this.var_1346.offerId);
            }
            return;
         }
         if(param1.type == WindowEvent.const_58)
