package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_334);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_367.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_367.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_367.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_367.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_367.view.setNewMessageArrived();
                  this.var_367.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_334).received = true;
         this.findCategory(FriendCategory.const_934).received = true;
         ...
         for each(_loc3_ in this.getCategoriesNotReceived())
         {
            ...
            {
               Util.remove(this.var_93,_loc3_);
               _loc3_.dispose();
            }
         }
      }
